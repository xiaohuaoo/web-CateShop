...
<script>
import { getAboutGoodslist, isUser, AddGoodsToCart } from '@/network/api'
...
export default {
  ...
  async created () {
    this.getPicturesList(this.$route.query.aboutgoodsId)
  },
  mounted () {},
  methods: {
    // 当前餐品信息
    async getPicturesList (aboutgoodsId) {
      const { aboutgoodslist } = await getAboutGoodslist({ aboutgoodsId })
      this.goodsInfo = aboutgoodslist[aboutgoodsId]
    },
    // 添加购物车
    async addGoods () {
