...
<script>
import { getGoodslist, isUser, AddGoodsToCart } from '@/network/api'
...
export default {
  ...
  async created () {
    this.getPicturesList(this.$route.query.goodsId)
  },
  mounted () {},
  methods: {
    // 当前餐品信息
    async getPicturesList (goodsId) {
      const { goodslist } = await getGoodslist({ goodsId })
      this.goodsInfo = goodslist[goodsId]
    },
    // 添加购物车
    async addGoods () {
